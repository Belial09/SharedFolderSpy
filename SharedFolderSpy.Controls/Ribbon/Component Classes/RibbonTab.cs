...
namespace Fesslersoft.SharedFolderSpy.Controls.Ribbon
{
    ...
    public class RibbonTab : Component, IRibbonElement, IRibbonToolTip, IContainsRibbonComponents
    {
        ...
        private readonly RibbonPanelCollection _panels;
        ...
        /// </summary>
        [DefaultValue(null), Description("A string field for associating custom data for this control")]
        ...
        public Size MeasureSize(object sender, RibbonElementMeasureSizeEventArgs e)
        {
            ...
            var textSize = TextRenderer.MeasureText(Text, Owner.RibbonTabFont);
            return textSize;
        }
        ...
        {
            var largeLarger = GetLargerPanel(RibbonElementSizeMode.Large);

            if (largeLarger != null) return largeLarger;

            var mediumLarger = GetLargerPanel(RibbonElementSizeMode.Medium);

            if (mediumLarger != null) return mediumLarger;

            var compactLarger = GetLargerPanel(RibbonElementSizeMode.Compact);

            if (compactLarger != null) return compactLarger;

            var overflowLarger = GetLargerPanel(RibbonElementSizeMode.Overflow);
        ...
        {
            var tigger = _tabBounds != tabBounds;
        ...
        {
            var trigger = _tabContentBounds != tabContentBounds;
        ...
        internal void UpdatePanelsRegions()
        {
            ...
            var curRight = TabContentBounds.Left + Owner.PanelPadding.Left + _offset;
            var curLeft = TabContentBounds.Right - Owner.PanelPadding.Right;
            var panelsTop = TabContentBounds.Top + Owner.PanelPadding.Top;
        ...
        {
            var w = 13;
            var scrBuffer = _scrollRightVisible;
            var sclBuffer = _scrollLeftVisible;
            var rrBuffer = _scrollRightBounds;
            var rlBuffer = _scrollLeftBounds;
